...
class DynamicCommunityTraking:
    ...
    def _add_new_dcomm(cls, dynamic_community: DY_COMMS, last_index: int, ts:int, comm: List[str], event="birth") -> int:
        ...
        index = last_index + 1
        dynamic_community[index] = [(comm, ts, event)]
        return index
    ...
    def _process_split(cls, dynamic_community: DY_COMMS, step_community: List[str], community_index: int,
                       last_index: int, ts: int) -> FRONT:
        last_index += 1

        dynamic_community[community_index][-1][2] = "split"

        dynamic_community[last_index] = dynamic_community[community_index][0:-2]
    ...
    def __init__(self, temporal_network: LoaderInterface, threshold: float, steps_to_die: int):
    ...
    def find_events(self, n_ts_min=0, n_ts_max=-1):
        ...
        for i, comm in enumerate(actual_snapshot.community_label_propagation()):
            comms_name = [actual_snapshot.vs[index]["name"] for index in comm]
            d_communities[i] = [(comms_name, 0, "birth")]
        ...
        for ts in range(1, n_ts):
            ...
            for comm in actual_snapshot.community_label_propagation():
                comms_name = [actual_snapshot.vs[index]["name"] for index in comm]
                ...
                if len(matched_comms) == 0:
                    ...
                    if dcomm_pair in matched_comms:
                        # splitting
                        update_front = DynamicCommunityTraking._process_split(d_communities, comms_name, dcomm_pair,
                                                                              last_c_index, ts)
                    else:
    ...
    def _process_dead_communities(self, dynamic_community: DY_COMMS, front: FRONT, ts: int) -> None:
        ...
        for comm_id in front.keys():
            ...
            if (ts - last_ts) >= self.steps_to_die:
